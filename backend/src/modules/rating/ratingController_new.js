...
const submitRating = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ success: true, message: 'Rating submitted successfully', data: ratingDoc });
  } catch (error) {
    next(error);
  }
};
...
const getUserRating = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ success: true, message: 'Rating fetched', data: rating });
  } catch (error) {
    next(error);
  }
};
...
const markHelpful = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({ success: true, message: 'Marked as helpful', data: rating });
  } catch (error) {
    next(error);
  }
};
...
module.exports = { submitRating, getMovieRatings, getUserRating, markHelpful, deleteRating };